...
export async function generateStaticParams() {
  const posts = await getAllPost("games");
...
const getPost = async (slug) => {
  let res = await axios(
    `${process.env.NEXT_API_DOMAIN}/wp-json/wp/v2/games/?slug=${slug}`,
    {
      auth: credentials,
    }
  );
...
function formatTitleWithStrong(title, keyword) {
  ...
  if (parts.length === 1) {
    return <h1 className="text-xl not-h1 lg:text-2xl">{title}</h1>;
  } else {
    return (
      <h1 className="text-xl not-h1 lg:text-2xl">
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `/games/${post.slug}`,
      languages: {
        "en-us": `/games/${post.slug}`,
      },
    },
  };
}
